    ...
    public static void bookTicket(Passenger p)
    {
        ...
        if((p.berthPreference.equals("L") && TicketBooker.availableLowerBerths > 0 )||
           ...
           (p.berthPreference.equals("U") && TicketBooker.availableUpperBerths > 0)||
        	(p.berthPreference.equals("SL") && TicketBooker.availablesideLowerBerths > 0)||
        	(p.berthPreference.equals("SU") && TicketBooker.availablesideUpperBerths > 0))
        {
            ...
            else if(p.berthPreference.equals("SL"))
            {
                System.out.println("Side Lower Berth Given");
                //call booking function in the TicketBooker class
                booker.bookTicket(p,(TicketBooker.sidelowerBerthsPositions.get(0)),"SL");
                //remove the booked position from available positions and also decrease available seats of that
                // particular type
                TicketBooker.sidelowerBerthsPositions.remove(0);
                TicketBooker.availablesideLowerBerths--;
            }
            else if(p.berthPreference.equals("SU"))
            {
        ...
        while(loop)
        {
            ...
            switch(choice)
            {
                ...
                case 1:
                {
                    ...
                    System.out.println("Enter Passenger name,age and berth preference (L,M,U,SL,SU)");
                    String name = s.next();
                    int age = s.nextInt();
